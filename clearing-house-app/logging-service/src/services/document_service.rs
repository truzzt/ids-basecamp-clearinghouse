...
impl DocumentService {
    ...
    pub(crate) async fn create_enc_document(&self, ch_claims: ChClaims, doc: Document) -> anyhow::Result<DocumentReceipt> {
        ...
        match self.db.exists_document(&doc.id).await {
            ...
            _ => {
                ...
                match self.key_api.generate_keys(ch_claims, doc.pid.clone(), doc.dt_id.clone()).await {
                    Ok(key_map) => {
                        keys = key_map;
                        debug!("got keys");
                    }
                    Err(e) => {
                        ...
                        return Err(anyhow!("Error while retrieving keys!")); // InternalError
                    }
                };
                ...
                match self.db.get_document_with_previous_tc(doc.tc).await {
                    ...
                    Ok(None) => {
                        if doc.tc == 0 {
                            ...
                            return Err(anyhow!("Error while creating the chain hash!")); // InternalError
                        }
                    }
                    Err(e) => {
    ...
    pub(crate) async fn get_enc_documents_for_pid(&self,
        ...
        let cts = match self.db.get_documents_for_pid(&dt_id, &pid, sanitized_page, sanitized_size, &sanitized_sort, &sanitized_date_from, &sanitized_date_to).await{
            Ok(cts) => cts,
            Err(e) => {
        ...
        if cts.is_empty(){
            ...
            let key_maps = match self.key_api.decrypt_multiple_keys(ch_claims, Some(pid),&key_cts_list).await{
                Ok(key_map) => {
                    key_map
                }
                Err(e) => {
                    error!("Error while retrieving keys from keyring: {:?}", e);
                    return Err(anyhow!("Error while retrieving keys from keyring")); // InternalError
                }
            };
        ...
        match self.db.get_document(&id, &pid).await{
            ...
            Ok(Some(ct)) => {
                match hex::decode(&ct.keys_ct){
                    Ok(key_ct) => {
                        // TODO: This method needs some attention, because keyring api called `create_service_token` on `ch_claims`
                        match self.key_api.decrypt_key_map(ch_claims, hex::encode_upper(key_ct), Some(pid), ct.dt_id.clone()).await{
                            Ok(key_map) => {
                                //TODO check the hash
