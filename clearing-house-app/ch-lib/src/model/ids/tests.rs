...
use crate::model::ids::message::{IdsMessage, DOC_TYPE};
...
fn test_doc_to_message_conversion() -> Result<()>{
    ...
    let expected_issued = InfoModelDateTime::new();
    ...
    doc_parts.push(DocumentPart::new(ISSUED.to_string(), serde_json::to_string(&expected_issued).ok()));
    ...
    let doc = Document::new(expected_pid.clone(),DOC_TYPE.to_string(), 3421, doc_parts);
    ...
    assert_eq!(m.issued, expected_issued);
...
fn test_message_to_doc_conversion() -> Result<()>{
    ...
    let expected_issued = serde_json::to_string(&issued).ok().unwrap();
    let expected_issuer_connector = String::from("test_issuer");
    let expected_model_version = String::from("test_model_version");
    let expected_payload = String::from("test_payload");
    let expected_payload_type = String::from("test_payload_type");
